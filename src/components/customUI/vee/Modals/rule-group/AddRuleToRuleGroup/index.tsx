...
const AddRuleToRuleGroup = () => {
    ...
    const { isLoading: ruleGroupLoading, refetch } = useGetRuleGroupDetailTableQuery({ id: ruleGroupId as string });
    const [getRules, { isLoading: tableLoading }] = useLazyGetAllRulesByNameQuery();
    const [addRuleToRuleGroup, { isLoading }] = useAddRuleToRuleGroupMutation();
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            toast({
                variant: "default",
                description: "Rule added to Rule Group successfully"
            })
        } catch (error) {
            ...
            toast({
                ...
                description: errorMsg?.description || "Failed to add rule to rule group",
            })
        }
    }, [ruleGroupId, rules, 
        addRuleToRuleGroup, toast, refetch, setOpen, setRules])

    return (
        <BaseModal
            open={open}
            setOpen={setOpen}
            title="Add a rule to rule group"
            dialogTitle="Add a rule to rule group"
        ...
        >
            ...
            <Button
                ...
                disabled={isLoading || ruleGroupLoading || tableLoading}
            >
                Update Rule Group
            </Button>
        </BaseModal>
...
export default AddRuleToRuleGroup