...
import {
    CommandInfoRecordTransformed, ExecuteCommandPayload
...
interface useSubmitCommandExecutionProps {
    ...
    deviceIdentifiers: MultiValue<Option>;
    resetCommandForm: () => void;
}

const useSubmitCommandExecution = ({
    command, deviceIdentifiers,
    resetCommandForm
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const argsValue = getCommandArgsValue(formData, selectedCommandArg, selectedCommandParameter);
        const apiPayload: ExecuteCommandPayload = {
            ...
            deviceInfo: {
                ...
                identifiers: deviceIdentifiers.map(item => item.value)
            },
            commandInfo: {
