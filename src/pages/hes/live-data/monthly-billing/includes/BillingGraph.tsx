...
const prepareChartData = (data: any) => {
  const transformDataForChart = (data: any[]) => {
    ...
    return {
      dates: sortedData.map(item => moment(item.data_timestamp).format('MMM YYYY')),
  ...
  const options: ApexOptions = {
    chart: {
      type: 'bar', // Column chart is the default for 'bar' type
      height: 350,
      toolbar: {
        show: false
      }
    ...
    plotOptions: {
      bar: {
        horizontal: false, // Set to false to ensure the bars are vertical (column chart)
        columnWidth: '50%',
        }
    },
    stroke: {
...
const BillingGraph = () => {
  ...
  useEffect(() => {
    if (data) {
      const transformedData = prepareChartData(data[0].billingMetrics);
        ...
        {isFetching ? (
          ...
          <div className="flex relative flex-col md:flex-row mt-8">
              ...
              <div className="flex-1 overflow-x-scroll">
                <div>
                  {chartData && <div className='p-5 rounded-lg bg-white h-[70vh] graph-border'><BarGraph title={'Monthly Range'} type={'bar'} data={chartData} /></div>}
                  </div>
              </div>
          </div>
      </div>
