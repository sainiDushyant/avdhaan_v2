...
const DayRangeInstantaneous = () => {
  ...
  const { data, isFetching, isError, error } = useGetLiveDataMetricsQuery({
    searchQuery: getUrlSearchParams(query, 'data_type=profile-instant')
  });
  ...
  return (
    <DateTimeBarGraph
      ...
      error={error}
      setQuery={setQuery}
      data={data?.profileInstantMetrics}
    />
  );
};

export default DayRangeInstantaneous;
